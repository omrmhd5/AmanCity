...
class UserService {
   ...
   */
  static async createOrUpdateUser({ firebaseUid, name, phone, email }) {
   ...
   */
  static async updateFcmToken(firebaseUid, fcmToken) {
    return User.findOneAndUpdate(
      { firebaseUid },
      { $set: { fcmToken } },
   ...
   */
  static async updateLocation(firebaseUid, lat, lng) {
    return User.findOneAndUpdate(
      { firebaseUid },
      {
        $set: {
   ...
   */
  static async updateProfile(firebaseUid, { name, phone }) {
    const fields = {};
   ...
   */
  static async updatePhone(firebaseUid, phone) {
    return User.findOneAndUpdate(
      { firebaseUid },
      { $set: { phone } },
      { new: true, upsert: true, runValidators: true },
   ...
   */
  static async findUsersNear(lat, lng, radiusKm) {
   ...
   */
  static async findUserByFirebaseUid(firebaseUid) {
   ...
   */
  static async searchUsers(query, excludeFirebaseUid) {
   ...
   */
  static async sendContactRequest(fromFirebaseUid, toUserId) {
   ...
   */
  static async removeTrustedContact(callerFirebaseUid, contactUserId) {
   ...
   */
  static async getAcceptedContacts(firebaseUid) {
  ...
  /**
   * Get all trusted contacts for a user (all statuses).
   */
  static async getTrustedContacts(firebaseUid) {
    const user = await User.findOne({ firebaseUid }).populate(
      "trustedContacts.userId",
      "name phone _id",
    );
    if (!user) throw new Error("User not found.");
    return user.trustedContacts.map((c) => ({
      userId: c.userId._id,
      name: c.userId.name,
      phone: c.userId.phone,
      status: c.status,
